...
export default async function LeadDetail({ params }: { params: { id: string } }) {
  ...
  return (
    <main className="container-wrap py-8">
      <a href="/admin" className="text-sm text-brand-700 underline">← Back</a>
      <h1 className="mt-2 text-2xl font-bold">{lead.name}</h1>
      <p>{lead.phone} • {lead.email || 'No email'}</p>
      <p>{lead.serviceType} • {lead.address}</p>
      {lead.preferredDate && <p>Preferred date: {lead.preferredDate.toDateString()}</p>}
      {lead.notes && <p className="mt-2">Notes: {lead.notes}</p>}
      {lead.photoUrl && <Image src={lead.photoUrl} alt="Lead upload" width={300} height={200} className="mt-3 rounded" />}

      <form action={updateLead} className="mt-4 space-y-3 rounded bg-white p-4 shadow max-w-lg">
        ...
        <label className="block">Internal notes
          <textarea name="internalNotes" defaultValue={lead.internalNotes || ''} className="mt-1 w-full rounded border p-2" rows={4} />
        </label>
